...
import { FindUserResponseDto, ModifyUserResponseDto, SnsUserResponseDto } from '../../axios/member/response/auth';
...
import { findUserRequest, snsUser , idCheckRequest} from "../../axios/MemberAxios.ts";
...
import SnsUserRequestDto from "../../axios/member/request/auth/sns-user.request.dto.ts";


const Sns_Membership = () => {
  ...
  const idRef = useRef<HTMLInputElement | null>(null);
    ...
    const findUserResponse = (ResponseBody : ResponseBody<FindUserResponseDto>) => {
        ...
        const {code , id , username , email , gender , useryear , birthday , address , subaddress } = ResponseBody;
    ...
    const snsUserResponse = (responseBody : SnsUserResponseDto | ResponseDto | null) => {
      if(!responseBody) return;
      const {code} = responseBody;
      if(code === ResponseCode.VALIDATION_FAIL){
        alert('필수 입력 목록을 입력해야 합니다.');
        return;
      }
      if(code === ResponseCode.DUPLICATE_ID) {
        alert("이미 가입된 sns 계정이 있습니다.");
        navigator("/");
        return;
      }
  
      if(code === ResponseCode.SUCCESS)navigator("/Join_Membership_finish");
    };
    ...
    const onModifyHandler = () => {
      ...
      if(modifyPassword !==modifyPasswordCheck){
        ...
        const requestBody: SnsUserRequestDto = {userId , newId, username ,  password , 
          email , gender , useryear , birthday, address , subaddress};
          snsUser(requestBody).then(snsUserResponse);
    }
    useEffect(() => {
      if (modifyAddress === "") {
    ...
    const idCheckResponse = (responseBody: IdCheckResponseDto | ResponseDto | null) => {
      if (!responseBody) return;
      const { code } = responseBody;
      if (code === ResponseCode.VALIDATION_FAIL) setIdMessage('아이디 검사 중입니다.');
      if (code === ResponseCode.DUPLICATE_ID) {
        setIdError(true);
    ...
    const subaddressHandler = (event) => {
      ...
      const okorno = newId && modifyPassword || passwordCheck || modifyAddress || modifySubAddress ? "ok_butten" : "no_buutten";

  return (
    <div className='Sns_Membership'>
        <div className='account_text'>sns 간편 회원가입</div>
        ...
        <div className='user_account'>
            ...
            <div className='lien'></div>
            <div className="text_good">필수 입력</div>
            <InputBox ref={idRef} title="아이디" placeholder="로그인할 아이디를 입력해주세요." type="text" value={newId} onChange={onIdChangeHandler} isErrorMessage={isIdError} 
            message={idMessage} buttonTitle="중복확인" onButtonClick={onIdButtonClickHandler} onKeydown={onIdKeyDownHandler} />
            <InputBox ref={passwordRef}  title="비밀번호 입력" placeholder="비밀번호를 입력해 주세요." type="password" value={modifyPassword} 
            ...
            isErrorMessage={isPasswordCheckError} message={passwordCheckMessage} onKeydown={onPasswordCheckKeyDownHandler}/>
              ...
              <div className='lien'></div>
            <div className="text_good">선택 입력</div>
            <form>
