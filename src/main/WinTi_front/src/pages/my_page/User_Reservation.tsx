  ...
  const findseller_user = () => {
    if (userId) {
      const fetchRegistrations = async () => {
        try {
          const response = await axios.get('http://localhost:8080/api/buy/registrations', { params: { userId } });
  ...
  return (
    <div className='contWrap'>
      <div className='user_Registration'>
        <h1 className="head">예매내역</h1>
        <h4 className="headSide">마이페이지 {'>'}<span>예매내역</span></h4>
        <div className="line" />
        <div style={{ textAlign: 'right', marginBottom: '10px' }} className="selectBox">
          <select onChange={handleSortChange} value={sortKey}>
            <option value="createDate">예매일</option>
            <option value="showId">예매번호</option>
            <option value="title">상품명</option>
            <option value="status">예매상태</option>
          </select>
        </div>
        <table>
          <thead>
          ...
          <tbody>
            {registrations.map((reg, index) => (
              <tr key={index}>
                ...
                <td>{reg.title}</td>
                <td>{reg.status}</td>
                <td><Link className="moveShow" to={`/My_page/User_Reservation/${reg.showId}`}><button className='myShowBtn'>상세보기</button></Link></td>
              </tr>
            ))}
          </tbody>
        </table>
